...
SocketListener.prototype.saveTask = function( index, name, url, status )
{
	var self = this;
	self.client.rpush( 'tasks', JSON.stringify( {'index':index, 'name':name, 'url':url, 'status':status } ), function(err, res){
			console.log( "redis", err, res );
	} );
};
SocketListener.prototype.getTasks = function(socket)
{	
	this.client.lrange('tasks', 0, this.grabbers.length, function(err, reply){
		if( err ) {
			console.log("Redis error", err );
			throw err;
		}
		if( reply )
		{
			console.log("on connect ", err, reply );
		}
	});
};
SocketListener.prototype.manageMessages = function()
{
	...
	self.io.sockets.on('connection', function (socket) {
		...
		socket.on('create_grabber', function(data){
			...
			self.saveTask( index, self.grabbers[index].name, 'http://pipes.yahoo.com/pipes/pipe.run?_id=74637623e9a48f9491059a63594ee9f4&_render=rss', self.grabbers[index].status );
			
			socket.emit('grabber_created', { message: 'Grabber program created ' + self.grabbers[index].name, index : index, name:self.grabbers[index].name, url:'http://pipes.yahoo.com/pipes/pipe.run?_id=74637623e9a48f9491059a63594ee9f4&_render=rss', 'status':self.grabbers[index].status } );
		});
		...
		socket.on('start_grabber', function (data) {
			...
			socket.emit('grabber_started', { message: 'Grabber program started ' + self.grabbers[index].name, index : index, name:self.grabbers[index].name } );
