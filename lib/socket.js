...
SocketListener.prototype.getGrabbersArray = function()
{
...
SocketListener.prototype.manageMessages = function()
{
	...
	self.getSocket().sockets.on('connection', function (socket) {
		...
		//// D E S T R O Y  T H E  G R A B B E R /////////////////
		socket.on('destroy', function(data){
			var index = data.index, arr = self.getGrabbersArray(), grab = self.getGrabber(index);
			arr.remove(index);
			console.log(JSON.stringify(data));
			self.client.lset('tasks', data.index, data.index, function(error, reply){
				self.client.lrem('tasks', 0, data.index, function(error, reply){
					console.log("REM:: ", error, reply);
				});
			})
			
			socket.emit("destroyed", {message: "Grabber destroyed "+grab.name, name: grab.name} );
			delete grab;
			console.log("Undefined??? ", grab);
		});
