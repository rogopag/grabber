...
SocketListener.prototype.manageMessages = function()
{
	...
	self.getSocket().sockets.on('connection', function (socket) {
		...
		//// K I L L  T H E  G R A B B E R /////////////////
		socket.on('destroy', function(data){
			var index = data.index, arr = self.getGrabbersArray(), grab = self.getGrabber(index);
			arr.remove(index);
			self.client.ltrim('tasks', index, index, function(error, reply){
				console.log("TRIM:: ", error, reply);
			});
			socket.emit("destroyed", {message: "Grabber destroyed "+grab.name, grab: data} );
			delete grab;
